...
SENSOR_STATES = [
    "HEALTHY", # GOOD SENSORS [CONSIDERED FOR NEXT TRAINIGN STEPS]
...
for t, row in df_sensors.iterrows():
    ...
    # ---------------- AVERAGES ----------------
    p_vals, t_vals, f_vals = [], [], []
    for s in main_sensor_list["P"]:
        if sensor_buffers[s]:
            p_vals.append(sensor_buffers[s][-1])
    for s in main_sensor_list["T"]:
        if sensor_buffers[s]:
            t_vals.append(sensor_buffers[s][-1])
    for s in main_sensor_list["F"]:
        if sensor_buffers[s]:
            f_vals.append(sensor_buffers[s][-1])

    if p_vals: avg_p_buf.append(np.mean(p_vals))
    if t_vals: avg_t_buf.append(np.mean(t_vals))
    if f_vals: avg_f_buf.append(np.mean(f_vals))

    # ---------------- PCA INPUT ----------------
    pca_row = []
    for k in ["P", "T", "F"]:
        for s in main_sensor_list[k]:
            if sensor_buffers[s]:
                pca_row.append(sensor_buffers[s][-1])

    if len(pca_row) >= PCA_COMPONENTS:
        pca_input_buffer.append(pca_row)

    # ---------------- PCA TRAIN / APPLY ----------------
    if len(pca_input_buffer) == WINDOW:
        X = scaler.fit_transform(np.array(pca_input_buffer))
        pca.partial_fit(X)

    if len(pca_input_buffer) >= WINDOW:
        x = scaler.transform([pca_input_buffer[-1]])
        z = pca.transform(x)
        xr = pca.inverse_transform(z)
        err = np.linalg.norm(x - xr)

        pca_scores_buffer.append(z[0])
        pca_recon_err_buffer.append(err)

    # ---------------- OPERATIONAL THRESHOLD ----------------
    op_energy = compute_operational_energy(row)
    operational_energy_buf.append(op_energy)

    update_threshold_regression(
        operational_energy_buf,
        pca_recon_err_buffer
    )

    dyn_threshold = alpha * op_energy + beta

    # ---------------- PLOTTING ----------------
    # Pressure / Temp / Flow
    for s in main_sensor_list["P"]:
        if s not in pressure_lines:
            pressure_lines[s] = ax_pressure.plot([], [], lw=1)[0]
        y = list(sensor_buffers[s])
        x = window_x(len(y))
        pressure_lines[s].set_data(x, y)

    ax_pressure.set_xlim(-WINDOW + 1, 0)
    ax_pressure.relim()
    ax_pressure.autoscale_view(scaley=True)
    
    # Temperature
    for s in main_sensor_list["T"]:
        if s not in temp_lines:
            temp_lines[s] = ax_temp.plot([], [], lw=1)[0]
        y = list(sensor_buffers[s])
        x = window_x(len(y))
        temp_lines[s].set_data(x, y)

    ax_temp.set_xlim(-WINDOW + 1, 0)
    ax_temp.relim()
    ...
    for s in main_sensor_list["F"]:
        if s not in flow_lines:
            flow_lines[s] = ax_flow.plot([], [], lw=1)[0]
        y = list(sensor_buffers[s])
        x = window_x(len(y))
        flow_lines[s].set_data(x, y)

    ax_flow.set_xlim(-WINDOW + 1, 0)
    ax_flow.relim()
    ...
    # PCA components
    if pca_scores_buffer:
        pcs = np.array(pca_scores_buffer)
        x = window_x(len(pcs))
        for i, k in enumerate(["PC1", "PC2", "PC3"]):
            pc_lines[k].set_data(x, pcs[:, i])

        ax_pca_rec.set_xlim(-WINDOW + 1, 0)
        ax_pca_rec.relim()
        ax_pca_rec.autoscale_view(scaley=True)
        pca_threshold_line.set_data(
    [-WINDOW + 1, 0],
    [dyn_threshold, dyn_threshold]
)

    # PCA loss
    if pca_recon_err_buffer:
        y = list(pca_recon_err_buffer)
        x = window_x(len(y))
        loss_line.set_data(x, y)
        ax_pca_loss.set_xlim(-WINDOW + 1, 0)
        ax_pca_loss.relim() 
        ax_pca_loss.autoscale_view(scaley=True)

    # Averages
    if avg_p_buf:
        avg_p_line.set_data(window_x(len(avg_p_buf)), avg_p_buf)
    if avg_t_buf:
        avg_t_line.set_data(window_x(len(avg_t_buf)), avg_t_buf)
    if avg_f_buf:
        avg_f_line.set_data(window_x(len(avg_f_buf)), avg_f_buf)

    ax_avg.set_xlim(-WINDOW + 1, 0)
    ax_avg.relim()
    ax_avg.autoscale_view(scaley=True)
